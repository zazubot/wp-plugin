...
class Zazubot_Public
{
  public function __construct()
  {
    ...
    add_action('wp_footer', array($this, 'initialize_zazubot'), 100);
  }
  ...
  public function add_zazubot_container($attributes = []) {
    $lib_version = ZAZUBOT_DEFAULT_LIB_VERSION;
    ...
    if (array_key_exists('zazubot', $attributes)) {
      $zazubot = $attributes['zazubot'];
      if (strlen($zazubot) > 50 || empty($zazubot) || !preg_match('/^[a-zA-Z0-9_-]+$/', $zazubot)) {
        return;
      } else {
        $zazubot = sanitize_text_field($zazubot);
      }
    }
    ...
    if (array_key_exists('ws_host', $attributes)) {
      ...
      if (strlen($ws_host) > 100 || !filter_var($ws_host, FILTER_VALIDATE_URL)) {
        $ws_host = 'partykit.zazubot.com'; // fallback to default host
      } else {
